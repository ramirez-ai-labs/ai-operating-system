...
from packages.shared.mcp.orchestrator_integration import MAX_TOOL_ROUNDS, run_with_mcp_tools
from packages.shared.providers.claude_provider import ClaudeProvider, ProviderResponse, get_provider
...
def _make_text_response(text: str, model: str = "claude-haiku-4-5-20251001") -> MagicMock:
...
def _make_tool_use_response(tool_id: str, tool_name: str, tool_input: dict, model: str = "claude-haiku-4-5-20251001") -> MagicMock:
...
def test_provider_complete_returns_text(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("packages.shared.providers.claude_provider._anthropic_sdk.Anthropic", return_value=mock_client):
...
def test_provider_complete_returns_error_response_on_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    mock_client = MagicMock()
...
def test_provider_context_injected_with_xml_delimiters(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
    mock_client = MagicMock()
...
def test_run_with_mcp_tools_returns_on_first_text_response(tmp_path) -> None:
    ...
    with patch("packages.shared.mcp.orchestrator_integration.ClaudeProvider", return_value=mock_provider):
...
def test_run_with_mcp_tools_executes_one_tool_round(tmp_path) -> None:
    ...
    with (
        patch("packages.shared.mcp.orchestrator_integration.ClaudeProvider", return_value=mock_provider),
        patch("packages.shared.mcp.orchestrator_integration.FilesystemMCPServer", return_value=mock_mcp),
    ):
...
def test_run_with_mcp_tools_fails_at_max_rounds(tmp_path) -> None:
    ...
    with (
        ...
        result = run_with_mcp_tools(prompt="go", data_path=str(tmp_path), max_rounds=MAX_TOOL_ROUNDS)
