#!/usr/bin/env python3
"""
One-on-One OS evaluation runner — Claude provider.

Runs the canonical eval set (evaluations/one_on_one_os/meeting_brief_cases.json)
against ClaudeOneOnOneProvider through the One-on-One OS graph and writes
scored results to evaluations/one_on_one_os/results_claude.json.
...
Usage:
    ...
    python scripts/run_one_on_one_os_evals_claude.py

    # CI (skips gracefully if key absent)
    python scripts/run_one_on_one_os_evals_claude.py --ci
"""
...
def run_evals(ci_mode: bool = False) -> int:
    ...
    base_cases = load_one_on_one_os_eval_cases()
    claude_cases: list[OneOnOneBriefEvalCase] = [
        case.model_copy(
            update={
                "inputs": case.inputs.model_copy(
                    update={"use_model": True, "provider": "claude", "claude_model": _CLAUDE_MODEL}
                )
            }
        )
    ...
    logger.info(
        "Running %d One-on-One OS eval cases against ClaudeOneOnOneProvider...\n",
        len(claude_cases),
    )

    results = run_local_one_on_one_os_evaluations(claude_cases)
    ...
    summary = {
        ...
        "provider": "claude",
        "model": _CLAUDE_MODEL,
        "eval_path": "evaluations/one_on_one_os/meeting_brief_cases.json",
        "summary": {
            "total": len(results),
            "passed": passed,
            "failed": failed,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run One-on-One OS evals against Claude.")
    parser.add_argument(
        "--ci",
        action="store_true",
