#!/usr/bin/env python3
"""
One-on-One OS evaluation runner — ChromaDB retrieval backend.

Runs the canonical eval set (evaluations/one_on_one_os/meeting_brief_cases.json)
with RETRIEVAL_BACKEND=chroma through the One-on-One OS graph and writes scored
results to evaluations/one_on_one_os/results_chroma.json.
...
Usage:
    ...
    python -m scripts.run_one_on_one_os_evals_chroma --ci
"""
...
def run_evals(ci_mode: bool = False) -> int:
    ...
    logger.info(
        "Running %d One-on-One OS eval cases with RETRIEVAL_BACKEND=chroma...\n",
        len(cases),
    )

    results = run_local_one_on_one_os_evaluations(
        cases, evaluators=ONE_ON_ONE_OS_CHROMA_EVALUATORS
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run One-on-One OS evals against ChromaDB.")
    parser.add_argument(
        "--ci",
        action="store_true",
