#!/usr/bin/env python3
"""
Interview OS evaluation runner — Claude provider.

Runs the canonical eval set (evaluations/interview_os/interview_cases.json)
against ClaudeInterviewProvider through the Interview OS graph and writes
scored results to evaluations/interview_os/results_claude.json.
...
Usage:
    ...
    python scripts/run_interview_os_evals_claude.py

    # CI (skips gracefully if key absent)
    python scripts/run_interview_os_evals_claude.py --ci
"""
...
def run_evals(ci_mode: bool = False) -> int:
    ...
    base_cases = load_interview_os_eval_cases()
    claude_cases: list[InterviewBriefEvalCase] = [
        case.model_copy(
            update={
    ...
    logger.info(
        "Running %d Interview OS eval cases against ClaudeInterviewProvider...\n",
        len(claude_cases),
    )

    results = run_local_interview_os_evaluations(claude_cases)
    ...
    summary = {
        ...
        "provider": "claude",
        "model": _CLAUDE_MODEL,
        "eval_path": "evaluations/interview_os/interview_cases.json",
        "summary": {
            "total": len(results),
            "passed": passed,
            "failed": failed,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Interview OS evals against Claude.")
    parser.add_argument(
        "--ci",
        action="store_true",
