#!/usr/bin/env python3
"""
Interview OS evaluation runner — ChromaDB retrieval backend.

Runs the canonical eval set (evaluations/interview_os/interview_cases.json)
with RETRIEVAL_BACKEND=chroma through the Interview OS graph and writes scored
results to evaluations/interview_os/results_chroma.json.
...
Usage:
    ...
    python -m scripts.run_interview_os_evals_chroma --ci
"""
...
def run_evals(ci_mode: bool = False) -> int:
    ...
    logger.info(
        "Running %d Interview OS eval cases with RETRIEVAL_BACKEND=chroma...\n",
        len(cases),
    )

    results = run_local_interview_os_evaluations(cases, evaluators=INTERVIEW_OS_CHROMA_EVALUATORS)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Interview OS evals against ChromaDB.")
    parser.add_argument(
        "--ci",
        action="store_true",
